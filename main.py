...
# Оптимизировано: 10s → 60s → 300s (настройки меняются редко, сброс кэша при изменении через веб)
# При изменении через веб вызывается invalidate_cache() — применяется мгновенно
